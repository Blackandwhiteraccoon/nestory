...
import { ViewState, InventoryItem, SubscriptionTier, TIER_CONFIG, SubscriptionState } from './types';
...
const App: React.FC = () => {
  ...
  const [subscription, setSubscription] = useState<SubscriptionState>({ tier: 'FREE' });
  ...
  const addToInventory = (item: InventoryItem) => {
    ...
    if (limits.maxItems !== 'unlimited' && inventoryItems.length >= limits.maxItems) {
      alert(`Limite atteinte ! Votre plan ${subscription.tier} est limité à ${limits.maxItems} objets. Passez au plan supérieur pour continuer.`);
  ...
  const getTitle = () => {
    switch (currentView) {
      case 'INVENTORY': return 'Mon Inventaire';
      case 'STOCK_MANAGER': return 'Gestion de Stock';
      case 'PRICE_UPDATER': return 'Audit Financier';
      case 'VISION': return 'Analyse & Scan';
      case 'VOICE_AGENT': return 'Assistant Vocal';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span className="font-bold text-xl tracking-tight text-white">Nexus<span className="text-blue-400">AI</span></span>
            </div>

            <div 
              onClick={() => setCurrentView('SUBSCRIPTION')}
              className={`ml-4 flex items-center gap-2 px-3 py-1.5 rounded-full border cursor-pointer transition-all ${
                subscription.tier === 'FREE' ? 'border-slate-700 bg-slate-800/50 text-slate-400' : 
                subscription.tier === 'BASE' ? 'border-blue-500/30 bg-blue-500/10 text-blue-400' :
                subscription.tier === 'PRO' ? 'border-purple-500/30 bg-purple-500/10 text-purple-400' :
                subscription.tier === 'EARLY_BIRD' ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' :
                'border-yellow-500/30 bg-yellow-500/10 text-yellow-400 font-bold'
              }`}
            >
              {subscription.isEarlyBird ? <Clock size={14} /> : <Crown size={14} />}
              <span className="text-xs uppercase tracking-wider">
                {subscription.tier}
                {subscription.expiresAt && ` (Exp: ${new Date(subscription.expiresAt).toLocaleDateString()})`}
              </span>
            </div>
